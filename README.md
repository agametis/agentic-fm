...
5. **Explode the XML** — run the **Explode XML** script to perform your first Save as XML export and populate `agent/xml_parsed/` in domain-first layout (`agent/xml_parsed/<domain>/<solution>/...`). Re-run it any time the solution schema changes.
...
1. In FileMaker, run "Explode XML" to export and parse the current solution into agent/xml_parsed/<domain>/<solution>/...
   (this also regenerates agent/context/<solution>/... and syncs the DuckDB index cache)
...
By default, CLI commands use `agent/CONTEXT.json.solution` when available. If a DuckDB session is already running for a different solution, stop it before switching solutions.
...
│   ├── context/             # Pre-extracted per-solution index files (generated by fmcontext.sh)
...
│   └── xml_parsed/          # Exploded XML organized as <domain>/<solution>/... (reference only)
...
- **agent/xml_parsed/** -- Exploded XML organized by domain and solution (`agent/xml_parsed/<domain>/<solution>/...`). Updated by `fmparse.sh` for the exported solution.
- **agent/context/** -- Compact, pipe-delimited indexes generated by `fmcontext.sh` under `agent/context/<solution>/`. Provide fast lookups of fields, relationships, layouts, scripts, table occurrences, and value lists.
...
- **duckdb/** -- Node.js ESM runtime for in-memory DuckDB indexing/search over solution-scoped XML and shared docs.
...
| **Explode XML**          | Saves a copy of the current solution as XML and calls `fmparse.sh` via the companion server to archive and explode it into `agent/xml_parsed/<domain>/<solution>/...`. Run this whenever the solution schema or scripts change. |
